# Project

## Contributing

Contributions of all kinds are welcome — bug reports, new indicators, PnL or metrics functions, documentation, and
fixes. The [contributing guide](https://github.com/ilpomo/pomata/blob/main/CONTRIBUTING.md) covers the setup and the
bar a change must clear: a new function ships with a docstring, an independent oracle, a declaration the shared rung set verifies, and
100% branch coverage. A feature is not done until it is verified.

## Changelog

Releases use GitHub's native release notes, grouped by label — see the
[releases page](https://github.com/ilpomo/pomata/releases). Pre-`1.0`, minor versions may change the API: every
breaking change carries the `breaking` label and is listed first in its release's notes, never landed silently.

## Citing pomata

If `pomata` supports your research, please cite it: the repository ships a
[`CITATION.cff`](https://github.com/ilpomo/pomata/blob/main/CITATION.cff) that GitHub renders into APA and BibTeX, and
the release is archived on Zenodo under DOI [`10.5281/zenodo.20980041`](https://doi.org/10.5281/zenodo.20980041).

## License

MIT — see [`LICENSE`](https://github.com/ilpomo/pomata/blob/main/LICENSE).
