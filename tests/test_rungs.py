...
@pytest.mark.parametrize("declaration", _DECLARATIONS, ids=_IDS)
def test_type_error(declaration: Declaration) -> None:
    """A bare column-name string in place of an expression raises the canonical ``TypeError``."""
    rungs.check_type_error(declaration)
