...
_METRICS = sorted(name for name, profile in REGISTRY.items() if profile.macro is Macro.METRICS)
...
def test_shape(name: str) -> None:
    """Verifies the output has the shape (scalar / series) its registry row declares, at ``Float64``."""
    contracts.assert_shape(getattr(metrics, name), REGISTRY[name])
