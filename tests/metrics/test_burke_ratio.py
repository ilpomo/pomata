"""
Tests for ``pomata.metrics.burke_ratio`` — the excess CAGR per unit of drawdown energy.

``burke_ratio`` is single-input and REDUCING (an equity series → one scalar), so tests read the single output row of
``apply_expr``; ``assert_matches`` and the naive ``burke_ratio_reference`` oracle are shared across the suite. It
reads a normalized growth-factor curve, so it is neither scale-homogeneous nor scale-invariant -- its correctness is
pinned by the reference, a golden master, and the metamorphic identity linking it to :func:`cagr` and :func:`drawdown`.
...
# ----------------------------------------------------------------------------------------------------------------------
# Test sizing -- burke_ratio is windowless and REDUCING (M = 0); a case is just a positive equity series. Facts:
...
#   3. scale   neither (a normalized growth factor over a scale-invariant drawdown energy) -> reference + identity
...
class TestBurkeRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(burke_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(burke_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS).alias("b"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [1.1, 1.05, 1.2, 1.15], dtype=pl.Float64)})
        expr = burke_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS).alias("b")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(burke_ratio(pl.col(COLUMN_X), periods_per_year=4).over(GROUP_KEY).alias("b"))[
            "b"
        ].to_list()
        expected_a = burke_ratio_reference(group_a, 4, 0.0)
        expected_b = burke_ratio_reference(group_b, 4, 0.0)
        assert_matches(
            grouped, [expected_a] * len(group_a) + [expected_b] * len(group_b), rel_tol=RELATIVE_TOLERANCE_REFERENCE
        )


class TestBurkeRatioEdge:
    """
    ...
    def test_periods_per_year_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="periods_per_year must be >= 1"):
            burke_ratio(pl.col(COLUMN_X), periods_per_year=0)
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], burke_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS)), [None])
...
class TestBurkeRatioCorrectness:
    ...
    def test_matches_reference(self) -> None:
        ...
        assert_matches(
            apply_expr(values, burke_ratio(pl.col(COLUMN_X), periods_per_year=4, risk_free_rate=0.02)),
            [burke_ratio_reference(values, 4, 0.02)],
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: the annual growth over the drawdown energy of the curve is 0.6776.
        """
        values = [1.1, 1.05, 1.2, 1.15, 1.3, 1.25, 1.4]
        assert_matches(apply_expr(values, burke_ratio(pl.col(COLUMN_X), periods_per_year=1).round(4)), [0.6776])


class TestBurkeRatioProperties:
    """
    ...
    def test_matches_reference_for_any_input(self, case: list[float], periods: int, risk_free: float) -> None:
        ...
        """
        assert_matches(
            apply_expr(case, burke_ratio(pl.col(COLUMN_X), periods_per_year=periods, risk_free_rate=risk_free)),
            [burke_ratio_reference(case, periods, risk_free)],
    ...
    def test_matches_component_definition(self, case: list[float], periods: int, risk_free: float) -> None:
        """
        Verifies the metamorphic identity: ``burke_ratio`` equals (:func:`cagr` minus the risk-free rate) divided by the
        square root of the sum of squared :func:`drawdown`, computed as separate metrics.
        """
        direct = apply_expr(case, burke_ratio(pl.col(COLUMN_X), periods_per_year=periods, risk_free_rate=risk_free))
        composed = apply_expr(
            case,
            (cagr(pl.col(COLUMN_X), periods_per_year=periods) - risk_free)
            / (drawdown(pl.col(COLUMN_X)) ** 2).sum().sqrt(),
        )
