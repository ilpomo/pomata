...
class TestProfitRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(profit_ratio(pl.col(COLUMN_X)), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(profit_ratio(pl.col(COLUMN_X)).alias("p"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [0.01, -0.02, 0.015, -0.03], dtype=pl.Float64)})
        expr = profit_ratio(pl.col(COLUMN_X)).alias("p")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(profit_ratio(pl.col(COLUMN_X)).over(GROUP_KEY).alias("p"))["p"].to_list()
        expected_a = profit_ratio_reference(group_a)
        expected_b = profit_ratio_reference(group_b)
        assert_matches(
...
class TestProfitRatioEdge:
    ...
    def test_empty(self) -> None:
        ...
        """
        assert_matches(apply_expr([], profit_ratio(pl.col(COLUMN_X))), [None])
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], profit_ratio(pl.col(COLUMN_X))), [None])
...
class TestProfitRatioProperties:
    ...
    def test_scale_invariance(self, case: list[float], exponent: int) -> None:
        """
        Verifies that a positive rescale of the returns leaves the profit factor unchanged (a ratio of sums), using
