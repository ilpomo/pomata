"""
Naive reference oracle for ``pomata.metrics.capture_downside_ratio``.
"""
...
def capture_downside_ratio_reference(
    ...
    an empty down-market). With no down-market period the result is ``None``. A zero annualized benchmark loss gives
    ``+/-inf`` (or ``nan``), matching the implementation's division.
    """
