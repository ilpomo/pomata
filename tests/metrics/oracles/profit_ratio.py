"""
Naive reference oracle for ``pomata.metrics.profit_ratio``.
"""
...
def profit_ratio_reference(returns: Sequence[float | None]) -> float | None:
    """
    ...
    the oracle for :func:`pomata.metrics.profit_ratio`. ``None`` returns are skipped; a ``nan`` anywhere poisons the
