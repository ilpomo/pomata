"""
Tests for ``pomata.metrics.ulcer_performance_ratio`` — the excess CAGR per unit of ulcer index (the Martin ratio).

``ulcer_performance_ratio`` is single-input and REDUCING (an equity series → one scalar), so tests read the single
output row of ``apply_expr``; ``assert_matches`` and the naive ``ulcer_performance_ratio_reference`` oracle (excess CAGR
over the ulcer index) are shared across the suite. It reads a normalized growth-factor curve, so it is neither
scale-homogeneous nor scale-invariant -- its correctness is pinned by the reference, a golden master, and the
metamorphic identity linking it to :func:`cagr` and :func:`ulcer_index`.
...
# ----------------------------------------------------------------------------------------------------------------------
# Test sizing -- ulcer_performance_ratio is windowless and REDUCING (M = 0); a case is a positive equity series. Facts:
...
#   3. scale   neither (a normalized growth factor over a scale-invariant ulcer index) -> pinned by reference + identity
...
class TestUlcerPerformanceRatioEdge:
    ...
    def test_periods_per_year_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="periods_per_year must be >= 1"):
            ulcer_performance_ratio(pl.col(COLUMN_X), periods_per_year=0)
...
class TestUlcerPerformanceRatioCorrectness:
    ...
    def test_matches_reference(self) -> None:
        ...
        assert_matches(
            apply_expr(values, ulcer_performance_ratio(pl.col(COLUMN_X), periods_per_year=4, risk_free_rate=0.02)),
            [ulcer_performance_ratio_reference(values, 4, 0.02)],
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: the annual growth (0.0492) over the ulcer index (0.0275) is 1.7927.
        """
        values = [1.1, 1.05, 1.2, 1.15, 1.3, 1.25, 1.4]
        assert_matches(
            apply_expr(values, ulcer_performance_ratio(pl.col(COLUMN_X), periods_per_year=1).round(4)), [1.7927]
        )
...
class TestUlcerPerformanceRatioProperties:
    ...
    def test_matches_reference_for_any_input(self, case: list[float], periods: int, risk_free: float) -> None:
        ...
        assert_matches(
            apply_expr(
                case, ulcer_performance_ratio(pl.col(COLUMN_X), periods_per_year=periods, risk_free_rate=risk_free)
            ),
            [ulcer_performance_ratio_reference(case, periods, risk_free)],
    ...
    def test_matches_component_definition(self, case: list[float], periods: int, risk_free: float) -> None:
        """
        Verifies the metamorphic identity: ``ulcer_performance_ratio`` equals (:func:`cagr` minus the risk-free rate)
        divided by :func:`ulcer_index`, computed as separate metrics.
        """
        direct = apply_expr(
            case, ulcer_performance_ratio(pl.col(COLUMN_X), periods_per_year=periods, risk_free_rate=risk_free)
        )
        composed = apply_expr(
            case, (cagr(pl.col(COLUMN_X), periods_per_year=periods) - risk_free) / ulcer_index(pl.col(COLUMN_X))
        )
