"""
Tests for ``pomata.metrics.capture_upside_ratio`` — how much of the benchmark's gain a portfolio captured in up markets.

``capture_upside_ratio`` is two-input and REDUCING (a return series and a benchmark series → one scalar), so tests read
the single output row of ``materialize``; ``assert_matches`` and the naive ``capture_upside_ratio_reference`` oracle
(the geometric annualized portfolio return over the geometric annualized benchmark return on positive-benchmark periods)
are shared across the suite. It is a ratio of two annualized geometric returns, neither scale-homogeneous nor
...
edge (validation / empty / no-up-market / null misalignment / NaN), correctness (vs the closed-form reference and a
...
# ----------------------------------------------------------------------------------------------------------------------
# Test sizing -- capture_upside_ratio is windowless and REDUCING (M = 0); a case is a list of (return, benchmark) pairs.
...
class TestCaptureUpsideContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(capture_upside_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(
            capture_upside_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS).alias("c")
        )
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        expr = capture_upside_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS).alias("c")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(
            capture_upside_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=4).over(GROUP_KEY).alias("c")
        )["c"].to_list()
        expected_a = capture_upside_ratio_reference(returns_a, benchmark_a, 4)
        expected_b = capture_upside_ratio_reference(returns_b, benchmark_b, 4)
        assert_matches(
...
class TestCaptureUpsideEdge:
    ...
    def test_periods_per_year_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="periods_per_year must be >= 1"):
            capture_upside_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=0)
    ...
    def test_no_up_market_is_null(self) -> None:
        """
        Verifies that with no positive-benchmark period the ratio is undefined, so the result is ``null``.
        """
        assert_matches(
            materialize(
...
class TestCaptureUpsideCorrectness:
    ...
    def test_matches_reference(self) -> None:
        ...
        assert_matches(
            ...
            [capture_upside_ratio_reference(returns, benchmark, PERIODS)],
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: the daily-annualized upside capture ratio is 1.3781.
        """
        returns = [0.012, -0.008, 0.02, -0.015, 0.005, 0.0, -0.02, 0.018]
        benchmark = [0.01, -0.006, 0.018, -0.012, 0.004, 0.002, -0.018, 0.015]
        assert_matches(
...
class TestCaptureUpsideProperties:
    ...
    def test_self_capture_is_one(self, case: list[tuple[float, float]], periods: int) -> None:
        """
        Verifies the metamorphic identity: a portfolio identical to its benchmark captures exactly one of its upside.
        """
