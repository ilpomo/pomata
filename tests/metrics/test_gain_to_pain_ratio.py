...
class TestGainToPainRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(gain_to_pain_ratio(pl.col(COLUMN_X)), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(gain_to_pain_ratio(pl.col(COLUMN_X)).alias("g"))
        assert result.height == 1
        assert result.schema["g"] == pl.Float64
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [0.01, -0.02, 0.015, -0.03], dtype=pl.Float64)})
        expr = gain_to_pain_ratio(pl.col(COLUMN_X)).alias("g")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(gain_to_pain_ratio(pl.col(COLUMN_X)).over(GROUP_KEY).alias("g"))["g"].to_list()
        expected_a = gain_to_pain_ratio_reference(group_a)
        expected_b = gain_to_pain_ratio_reference(group_b)
        assert_matches(
...
class TestGainToPainRatioEdge:
    ...
    def test_empty(self) -> None:
        ...
        """
        assert_matches(apply_expr([], gain_to_pain_ratio(pl.col(COLUMN_X))), [None])
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], gain_to_pain_ratio(pl.col(COLUMN_X))), [None])
...
class TestGainToPainRatioProperties:
    ...
    def test_scale_invariant(self, case: list[float], exponent: int) -> None:
        """
        Verifies that a positive rescale of the returns leaves the gain to pain ratio unchanged (a ratio of sums),
