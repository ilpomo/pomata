"""
Tests for ``pomata.metrics.payoff_ratio`` — the average winning return over the average losing return.

``payoff_ratio`` is single-input and REDUCING (a return series → one scalar), so tests read the single output row of
``apply_expr``; ``assert_matches`` and the naive ``payoff_ratio_reference`` oracle (mean win over mean loss) are shared
across the suite. It is scale-invariant (a ratio of means), so it carries a scale-invariance tier.
...
# ----------------------------------------------------------------------------------------------------------------------
# Test sizing -- payoff_ratio is windowless and REDUCING (M = 0); a case is just a return series. Facts:
...
#   3. scale   invariant (a ratio of means) -> scale-invariance tier
...
class TestPayoffRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(payoff_ratio(pl.col(COLUMN_X)), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(payoff_ratio(pl.col(COLUMN_X)).alias("p"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [0.01, -0.02, 0.015, -0.03], dtype=pl.Float64)})
        expr = payoff_ratio(pl.col(COLUMN_X)).alias("p")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(payoff_ratio(pl.col(COLUMN_X)).over(GROUP_KEY).alias("p"))["p"].to_list()
        expected_a = payoff_ratio_reference(group_a)
        expected_b = payoff_ratio_reference(group_b)
        assert_matches(
            grouped, [expected_a] * len(group_a) + [expected_b] * len(group_b), rel_tol=RELATIVE_TOLERANCE_REFERENCE
        )


class TestPayoffRatioEdge:
    """
    ...
    def test_empty(self) -> None:
        ...
        """
        assert_matches(apply_expr([], payoff_ratio(pl.col(COLUMN_X))), [None])
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], payoff_ratio(pl.col(COLUMN_X))), [None])
    ...
    def test_null_skipped(self) -> None:
        ...
        assert_matches(
            apply_expr(values, payoff_ratio(pl.col(COLUMN_X))),
...
class TestPayoffRatioCorrectness:
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: average win (0.01625) over average loss magnitude (0.015) is 1.0833.
        """
        values = [0.03, -0.01, 0.02, -0.015, 0.01, 0.005, -0.02]
        assert_matches(apply_expr(values, payoff_ratio(pl.col(COLUMN_X)).round(4)), [1.0833])


class TestPayoffRatioProperties:
    """
    ...
    def test_matches_reference_for_any_input(self, case: list[float]) -> None:
        ...
        """
        assert_matches(
            apply_expr(case, payoff_ratio(pl.col(COLUMN_X))),
            [payoff_ratio_reference(case)],
    ...
    def test_scale_invariant(self, case: list[float], exponent: int) -> None:
        ...
        scaled = apply_expr([value * k for value in case], payoff_ratio(pl.col(COLUMN_X)))
