"""
Tests for ``pomata.metrics.treynor_ratio`` — the annualized excess return per unit of systematic (benchmark) risk.

``treynor_ratio`` is two-input and REDUCING (a return series and a benchmark series → one scalar), so tests read the
single output row of ``materialize``; ``assert_matches`` and the naive ``treynor_ratio_reference`` oracle (annualized
arithmetic excess return over beta, on pairwise-complete observations) are shared across the suite. It annualizes a
return over a slope, so it is neither scale-homogeneous nor scale-invariant -- its correctness is pinned by the
reference, a golden master, and the metamorphic identity linking it to :func:`beta`.

The ladder is the canonical one: contract (type / reduces-to-scalar / lazy-eager / ``.over`` per-group independence),
edge (validation / empty / single-pair / zero-beta / null misalignment / NaN), correctness (vs the closed-form reference
and a frozen golden master), and properties (reference agreement incl. missing data, the component-definition identity).
...
# ----------------------------------------------------------------------------------------------------------------------
# Test sizing -- treynor is windowless and REDUCING (M = 0); a case is a list of (return, benchmark) pairs. Facts:
...
#   3. scale   neither (an annualized excess return over a slope) -> reference + component identity
...
def _beta_nondegenerate(returns: Sequence[float | None], benchmark: Sequence[float | None]) -> bool:
    """
    Whether the regression slope (beta) is bounded away from zero: treynor divides the excess mean by beta, so constant
    or subnormal returns (beta toward zero) make the one-pass quotient diverge from the oracle's exact ``inf``.
    """
    pairs = [
    ...
    complete_benchmark = [value for _, value in pairs]
    mean_returns = sum(complete_returns) / len(pairs)
    mean_benchmark = sum(complete_benchmark) / len(pairs)
    variance = sum((value - mean_benchmark) ** 2 for value in complete_benchmark) / len(pairs)
    if variance == 0.0:
        return False
    covariance = sum(
        (value_returns - mean_returns) * (value_benchmark - mean_benchmark)
        for value_returns, value_benchmark in zip(complete_returns, complete_benchmark, strict=True)
    ) / len(pairs)
...
class TestTreynorRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS).alias("t"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        expr = treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS).alias("t")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(
            treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=4).over(GROUP_KEY).alias("t")
        )["t"].to_list()
        expected_a = treynor_ratio_reference(returns_a, benchmark_a, 4, 0.0)
        expected_b = treynor_ratio_reference(returns_b, benchmark_b, 4, 0.0)
        assert_matches(
...
class TestTreynorRatioEdge:
    ...
    def test_periods_per_year_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="periods_per_year must be >= 1"):
            treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=0)
    ...
    def test_non_finite_risk_free_rate_raises(self) -> None:
        ...
        for invalid in (math.nan, math.inf, -math.inf):
            with pytest.raises(ValueError, match="risk_free_rate must be a finite number"):
                treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS, risk_free_rate=invalid)
    ...
    def test_empty(self) -> None:
        ...
        assert_matches(
            materialize(
                {RETURNS: [], BENCHMARK: []},
                treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS),
            ),
            [None],
        )
    ...
    def test_zero_beta_is_inf(self) -> None:
        """
        Verifies that a zero beta (a return uncorrelated with the benchmark) with a positive excess return gives
        ``+inf``, reported rather than clipped. Integer-valued returns keep the covariance exactly zero.
        """
        result = materialize(
            {RETURNS: [3.0, 3.0, 1.0, 1.0], BENCHMARK: [1.0, -1.0, 1.0, -1.0]},
            treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=PERIODS),
        )
    ...
    def test_constant_benchmark_is_nan(self) -> None:
        """
        Verifies that a constant (zero-variance) benchmark makes the embedded beta ``NaN`` (via the ``max == min``
        guard, regardless of the constant's magnitude), so the excess-over-beta ratio is ``NaN``.
        """
...
class TestTreynorRatioCorrectness:
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: the daily-annualized Treynor ratio at a 2% risk-free rate is 0.3083.
        """
        returns = [0.012, -0.008, 0.02, -0.015, 0.005, 0.0, -0.02, 0.018]
        benchmark = [0.01, -0.006, 0.018, -0.012, 0.004, 0.002, -0.018, 0.015]
        assert_matches(
            materialize(
                {RETURNS: returns, BENCHMARK: benchmark},
                treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=252, risk_free_rate=0.02).round(4),
            ),
            [0.3083],
        )
...
class TestTreynorRatioProperties:
    ...
    def test_matches_reference_for_any_input(
        ...
        assume(well_spread(complete_benchmark(returns, benchmark)) and _beta_nondegenerate(returns, benchmark))
        assert_matches(
            materialize(
                {RETURNS: returns, BENCHMARK: benchmark},
                treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=periods, risk_free_rate=risk_free),
            ),
            [treynor_ratio_reference(returns, benchmark, periods, risk_free)],
    ...
    def test_matches_component_definition(
        ...
        """
        Verifies the metamorphic identity: ``treynor_ratio`` equals the annualized arithmetic excess return divided by
        the public :func:`beta`, computed as separate metrics.
        """
        returns, benchmark = split_pairs(case)
        assume(well_spread(complete_benchmark(returns, benchmark)))
        rf_period = math.pow(1.0 + risk_free, 1.0 / periods) - 1.0
        annualized_excess = (pl.col(RETURNS) - rf_period).mean() * periods
        composed = annualized_excess / beta(pl.col(RETURNS), pl.col(BENCHMARK))
        direct = materialize(
            {RETURNS: returns, BENCHMARK: benchmark},
            treynor_ratio(pl.col(RETURNS), pl.col(BENCHMARK), periods_per_year=periods, risk_free_rate=risk_free),
        )
