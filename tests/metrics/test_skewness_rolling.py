...
class TestSkewnessRollingEdge:
    ...
    def test_window_exceeds_length(self) -> None:
        ...
        """
        values = [0.01, -0.02, 0.03, -0.01, 0.02]
        assert_matches(apply_expr(values, skewness_rolling(pl.col(COLUMN_X), 7)), [None, None, None, None, None])
    ...
    def test_window_equals_length(self) -> None:
        ...
        values = [0.01, -0.02, 0.03, -0.01, 0.02]
        assert_matches(
            apply_expr(values, skewness_rolling(pl.col(COLUMN_X), 5)),
            skewness_rolling_reference(values, 5),
            rel_tol=RELATIVE_TOLERANCE_REFERENCE,
        )
