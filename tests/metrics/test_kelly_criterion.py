...
class TestKellyCriterionContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(kelly_criterion(pl.col(COLUMN_X)), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(kelly_criterion(pl.col(COLUMN_X)).alias("k"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [0.01, -0.02, 0.015, -0.03], dtype=pl.Float64)})
        expr = kelly_criterion(pl.col(COLUMN_X)).alias("k")
        assert_frame_equal(frame.select(expr), frame.lazy().select(expr).collect())

    def test_over_partitions_independently(self) -> None:
        """
        Verifies that under ``.over`` the fraction is computed per group (broadcast) and never spans boundaries.
        """
        ...
        grouped = frame.select(kelly_criterion(pl.col(COLUMN_X)).over(GROUP_KEY).alias("k"))["k"].to_list()
        expected_a = kelly_criterion_reference(group_a)
        expected_b = kelly_criterion_reference(group_b)
        assert_matches(
...
class TestKellyCriterionEdge:
    ...
    def test_empty(self) -> None:
        ...
        """
        assert_matches(apply_expr([], kelly_criterion(pl.col(COLUMN_X))), [None])
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], kelly_criterion(pl.col(COLUMN_X))), [None])
...
class TestKellyCriterionProperties:
    ...
    def test_scale_invariance(self, case: list[float], exponent: int) -> None:
        """
        Verifies that a positive rescale of the returns leaves the Kelly fraction unchanged (its inputs are
        scale-invariant), using powers of two so the rescaling is lossless.
        """
        k = 2.0**exponent
