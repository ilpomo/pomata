...
class TestSharpeRatioContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS), pl.Expr)
    ...
    def test_reduces_to_scalar(self) -> None:
        ...
        result = frame.select(sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS).alias("s"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [0.01, -0.02, 0.015, -0.03], dtype=pl.Float64)})
        expr = sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS).alias("s")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS).over(GROUP_KEY).alias("s"))[
            "s"
        ].to_list()
        expected_a = sharpe_ratio_reference(group_a, PERIODS, 0.0)
        expected_b = sharpe_ratio_reference(group_b, PERIODS, 0.0)
        assert_matches(
...
class TestSharpeRatioEdge:
    ...
    def test_empty(self) -> None:
        ...
        """
        assert_matches(apply_expr([], sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS)), [None])
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_expr([None, None], sharpe_ratio(pl.col(COLUMN_X), periods_per_year=PERIODS)), [None])
