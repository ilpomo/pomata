...
class TestSharpeRatioProperties:
    ...
    def test_scale_invariance(self, case: list[float], exponent: int) -> None:
        """
        Verifies that ``sharpe_ratio`` is scale-invariant: scaling every input value by a constant ``k`` leaves the
        output unchanged -- ``sharpe_ratio(k * x) == sharpe_ratio(x)``. ``k`` is a power of two, so the rescale is
