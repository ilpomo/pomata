...
class TestInformationRatioRollingContract:
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(
            information_ratio_rolling(pl.col(RETURNS), pl.col(BENCHMARK), 3, periods_per_year=PERIODS), pl.Expr
        )
    ...
    def test_preserves_length_and_dtype(self) -> None:
        ...
        result = frame.select(
            information_ratio_rolling(pl.col(RETURNS), pl.col(BENCHMARK), 3, periods_per_year=PERIODS).alias("i")
        )
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        expr = information_ratio_rolling(pl.col(RETURNS), pl.col(BENCHMARK), 3, periods_per_year=PERIODS).alias("i")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        grouped = frame.select(
            information_ratio_rolling(pl.col(RETURNS), pl.col(BENCHMARK), 2, periods_per_year=PERIODS)
            .over(GROUP_KEY)
            .alias("i")
        )["i"].to_list()
        expected = information_ratio_rolling_reference(returns_a, benchmark_a, 2, PERIODS) + (
            information_ratio_rolling_reference(returns_b, benchmark_b, 2, PERIODS)
        )
