...
RISK_OF_RUIN = Spec(
    ...
    # (test_risk_of_ruin.py::test_scale_invariance).
    ...
    pins=(
        SpecPin(
            ...
            inputs={"returns": (0.05,)},
            expected=(0.0,),
            reason="a single win (p=1) gives ruin 0 (test_risk_of_ruin.py::test_single_row)",
        ),
        SpecPin(
            label="all_wins_is_zero",
            inputs={"returns": (0.01, 0.02, 0.03)},
            expected=(0.0,),
            reason="an all-winning series (p=1) has no ruin risk (test_risk_of_ruin.py::test_all_wins_is_zero)",
        ),
        SpecPin(
            label="all_losses_is_one",
            inputs={"returns": (-0.01, -0.02, -0.03)},
            expected=(1.0,),
            reason="an all-losing series (p=0) is certain ruin (test_risk_of_ruin.py::test_all_losses_is_one)",
        ),
        SpecPin(
            ...
            "(test_risk_of_ruin.py::test_all_zero_is_null)",
        ),
    ),
)
