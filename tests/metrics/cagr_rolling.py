...
CAGR_ROLLING = Spec(
    ...
    # annualized growth is scale-invariant (verified numerically).
    ...
    pins=(
        ...
        SpecPin(
            ...
            "zero START endpoint blows the ratio to +inf, reported not clipped; this pin sits exactly on the "
            "boundary, so the factory's <= 0 guard (not < 0) is load-bearing and no random draw holds it",
