...
class TestTailRatioProperties:
    ...
    def test_scale_invariance(self, case: list[float], exponent: int) -> None:
        """
        Verifies that ``tail_ratio`` is scale-invariant: scaling every input value by a constant ``k`` leaves the
        output unchanged -- ``tail_ratio(k * x) == tail_ratio(x)``. ``k`` is a power of two, so the rescale is exact
