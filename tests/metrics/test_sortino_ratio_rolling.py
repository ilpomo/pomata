...
class TestSortinoRatioRollingEdge:
    ...
    def test_window_exceeds_length(self) -> None:
        ...
        values = [0.01, -0.02, 0.03, -0.01, 0.02]
        assert_matches(
            apply_expr(values, sortino_ratio_rolling(pl.col(COLUMN_X), 7, periods_per_year=PERIODS)),
    ...
    def test_window_equals_length(self) -> None:
        ...
        values = [0.01, -0.02, 0.03, -0.01, 0.02]
        assert_matches(
            apply_expr(values, sortino_ratio_rolling(pl.col(COLUMN_X), 5, periods_per_year=PERIODS)),
            sortino_ratio_rolling_reference(values, 5, PERIODS),
            rel_tol=RELATIVE_TOLERANCE_REFERENCE,
        )
    ...
    def test_flat_zero_excess_window_by_slide_is_nan(self) -> None:
        ...
        incremental running-sum residue cannot ride above the exactly-zero downside deviation as a spurious ``inf``.
        """
        values = [-0.3233, -0.6457, 0.0, 0.4404, 0.0, 0.0, 0.0, 0.0]
        result = apply_expr(values, sortino_ratio_rolling(pl.col(COLUMN_X), 4, periods_per_year=PERIODS))
