...
def test_factories_are_typed_as_expr() -> None:
    ...
    assert_type(cost_borrow(quantity, price, 0.01), pl.Expr)
    assert_type(cost_fixed(quantity, 1.0), pl.Expr)
    assert_type(cost_funding(quantity, price, series), pl.Expr)
    assert_type(cost_notional(quantity, price, 0.01), pl.Expr)
    assert_type(cost_per_share(quantity, 1.0), pl.Expr)
    assert_type(cost_proportional(weight, 0.001), pl.Expr)
    assert_type(cost_slippage(weight, 0.0005), pl.Expr)
