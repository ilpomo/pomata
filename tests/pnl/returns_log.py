...
RETURNS_LOG = suite_pnl(
    ...
    examples=(
        ...
        Example(
            inputs={"price": (10.0, -5.0, 0.0, 5.0)},
            intro="**Domain** — a negative price relative is outside the logarithm's domain and returns "
            "``NaN``, a zero relative logs to ``-inf``, and a zero previous price makes the relative "
            "``+inf``:",
        ),
        Example(
            inputs={"price": (-0.0, 5.0)},
            intro="**Degenerate denominator** — a ``-0.0`` previous price flips the relative's sign, so a "
            "positive price gives a negative relative and the log is ``NaN``:",
        ),
        Example(
            inputs={"price": (-0.0, -5.0)},
            intro="**Degenerate denominator** — a ``-0.0`` previous price with a negative price gives a "
            "positive relative, so the log is ``+inf``:",
        ),
        Example(
            ...
            "indeterminate ``inf / inf``, so the second log return is ``NaN``:",
        ),
    ),
)
