...
contract parametrizes over the pnl subset of the registry (:mod:`tests.support.registry`) and applies the shared,
shape-aware assertions in :mod:`tests.support.contracts`. It is shared *within* pnl, never *across* families: the module
iterates pnl's own registry rows and tests each function by its own row; a newly added pnl function is swept in
automatically.
...
def test_shape(name: str) -> None:
    ...
    contracts.assert_shape(getattr(pnl, name), REGISTRY[name])
...
def test_empty(name: str) -> None:
    ...
    contracts.assert_empty(getattr(pnl, name), REGISTRY[name])


@pytest.mark.parametrize("name", _PNL)
def test_all_null(name: str) -> None:
    """Verifies an all-null series stays all-null."""
    contracts.assert_all_null(getattr(pnl, name), REGISTRY[name])
