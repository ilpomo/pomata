...
def _assert_reducing_flow(spec: Spec, clean: pl.DataFrame, injection: int, poisoned: pl.Series, *, nan: bool) -> None:
    if nan:  # POISONS: the scalar goes NaN
        ...
        return
    # SKIPPED: the scalar is what it would be if the row were absent. The masked evaluation and the dropped-row
    # evaluation may reduce in different SIMD orders on some platforms (a pairwise metric sums a masked lane vs a
    # contiguous one), so the comparison is kind-aware at the EXACT tolerance, never bit-for-bit.
    ...
    assert_matches(poisoned.to_list(), expected.to_list())
...
def test_pinned_cases(spec: Spec, pin: SpecPin) -> None:
    """Verifies each crafted-input case: the pinned frame maps to the pinned lanes (rounded where declared)."""
