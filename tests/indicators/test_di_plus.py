"""
Tests for ``pomata.indicators.di_plus`` — the Plus Directional Indicator (+DI).

``di_plus`` is multi-input (high, low, close), so tests use a local ``apply_di_plus`` helper to materialize the factory
over a three-column ``Float64`` frame; ``assert_matches`` and the naive ``di_plus_reference`` oracle are shared across
...
def apply_di_plus(
    ...
    """
    Materialize ``di_plus`` over a three-column ``Float64`` frame built from the aligned HLC lists.
    """
    return materialize({HIGH: high, LOW: low, CLOSE: close}, di_plus(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window))


class TestDiPlusContract:
    """
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        expr = di_plus(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), 2).over(GROUP_KEY)
        grouped = frame.select(expr.alias("y"))["y"].to_list()
        group_a = apply_di_plus([10.0, 11.0, 12.0, 11.5], [9.0, 10.0, 11.0, 10.5], [9.5, 10.5, 11.5, 11.0], 2)
        group_b = apply_di_plus([20.0, 21.0, 22.0, 21.5], [19.0, 20.0, 21.0, 20.5], [19.5, 20.5, 21.5, 21.0], 2)
        assert_matches(grouped, group_a + group_b)


class TestDiPlusEdge:
    """
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(apply_di_plus([None] * 4, [None] * 4, [None] * 4, 2), [None, None, None, None])

    def test_single_row(self) -> None:
        """
        Verifies that a one-row series with ``window > 1`` is all warm-up (one null).
        """
        assert_matches(apply_di_plus([10.0], [9.0], [9.5], 2), [None])

    def test_window_exceeds_length(self) -> None:
        """
        Verifies that a window exceeding the series length yields an all-null output.
        """
        assert_matches(apply_di_plus([10.0, 11.0, 12.0], [9.0, 10.0, 11.0], [9.5, 10.5, 11.5], 5), [None, None, None])

    def test_warmup_null_count(self) -> None:
        """
        Verifies that the first ``window - 1`` rows are null (warm-up).
        """
        result = apply_di_plus([10.0, 11.0, 12.0, 11.5], [9.0, 10.0, 11.0, 10.5], [9.5, 10.5, 11.5, 11.0], 2)
    ...
    def test_null_bridged(self) -> None:
        ...
        assert_matches(apply_di_plus(high, low, close, 2), di_plus_reference(high, low, close, 2))

    def test_nan_latches(self) -> None:
        """
        Verifies that a NaN propagates (matching the naive reference).
        """
        high = [10.0, 11.0, 12.0, 11.5, 13.0, math.nan, 14.0, 13.5]
        low = [9.0, 10.0, 11.0, 10.5, 12.0, 11.5, 13.0, 12.5]
        close = [9.5, 10.5, 11.5, 11.0, 12.5, 12.0, 13.5, 13.0]
        assert_matches(apply_di_plus(high, low, close, 2), di_plus_reference(high, low, close, 2))
...
class TestDiPlusCorrectness:
    ...
    def test_matches_reference(self) -> None:
        ...
        for window in (1, 2, 3, 5):
            assert_matches(
                apply_di_plus(high, low, close, window),
                di_plus_reference(high, low, close, window),
...
class TestDiPlusProperties:
    ...
    def test_scale_invariance(
        ...
        rows, window = case
        high, low, close = split_triples(rows)
        base = apply_di_plus(high, low, close, window)
        scaled = apply_di_plus(
    ...
    def test_bounded(
        ...
        rows, window = case
        high, low, close = split_triples(rows)
        for value in apply_di_plus(high, low, close, window):
