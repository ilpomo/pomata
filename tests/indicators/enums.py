...
windowed or recursive output warms up, how its recursion seeds (docstring metadata, see :class:`Seeding`), and its
relation to the TA-Lib reference (:class:`RelationTalib`), which drives the differential tier and carries the TA-Lib
partition as declared data.
"""
...
class Seeding(enum.Enum):
    """
    How a recursion seeds its first state — docstring metadata, not load-bearing.

    No rung reads this axis; it is recorded so the generated docstring can state each recursion's initialization in
    prose. Its correctness is not proven by the suite, so a declaration sets it only where the seeding is documented
    (the EWM means) and leaves ``NONE`` elsewhere rather than guessing.
    """
...
class RelationTalib(enum.Enum):
    """
    An indicator's relation to its TA-Lib twin — the partition the differential tier reads, carrying the no-twin /
    divergence data as declared fields.
