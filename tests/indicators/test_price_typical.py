"""
Tests for ``pomata.indicators.price_typical`` — Typical Price.

``price_typical`` is multi-input (``high`` / ``low`` / ``close``) and elementwise (no window, no cross-bar state).
Tests use a local ``apply_price_typical`` helper to materialize it over a three-column ``Float64`` frame;
``assert_matches`` and the naive ``price_typical_reference`` oracle are shared across the suite.
...
# price_typical is a windowless equal-weighted mean of high, low, and close, homogeneous of degree 1 (so it keeps the
...
def _cases[T](draw: st.DrawFn, bars: st.SearchStrategy[T], min_size: int = 1) -> list[T]:
    """
    A series of bars sized from the facts above. price_typical is windowless (W = 0), so -- unlike the windowed
...
def apply_price_typical(
    high: Sequence[float | None],
    low: Sequence[float | None],
    close: Sequence[float | None],
) -> list[float | None]:
    """
    Materialize ``price_typical`` over a three-column ``Float64`` frame from the aligned input lists.
    """
    return materialize({HIGH: high, LOW: low, CLOSE: close}, price_typical(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE)))


class TestPriceTypicalContract:
    """
    ...
    def test_over_is_identity(self) -> None:
        ...
        plain = frame.select(price_typical(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE)).alias("y"))["y"].to_list()
        grouped = frame.select(price_typical(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE)).over("ticker").alias("y"))[
            "y"
        ].to_list()
        assert_matches(plain, grouped)
        assert_matches(
            plain,
            [10.0, 11.166666666666666, 12.166666666666666, 11.666666666666666, 13.166666666666666, 14.166666666666666],
        )


class TestPriceTypicalEdge:
    """
    ...
    def test_all_null(self) -> None:
        ...
        """
        assert_matches(
            apply_price_typical([None, None, None], [None, None, None], [None, None, None]), [None, None, None]
        )
    ...
    def test_single_row(self) -> None:
        ...
        """
        assert_matches(apply_price_typical([11.0], [9.0], [10.0]), [10.0])

    def test_constant_series(self) -> None:
        """
        Verifies that when every input price equals the same constant the result is that constant on every row.
        """
        assert_matches(apply_price_typical([5.0, 5.0, 5.0], [5.0, 5.0, 5.0], [5.0, 5.0, 5.0]), [5.0, 5.0, 5.0])
    ...
    def test_null_propagates(self) -> None:
        ...
        """
        assert_matches(
            apply_price_typical([11.0, None, 13.0], [9.0, 10.0, 11.0], [10.0, 11.5, 12.5]),
            [10.0, None, 12.166666666666666],
        )
    ...
    def test_nan_propagates(self) -> None:
        ...
        """
        assert_matches(
            apply_price_typical([11.0, float("nan"), 13.0], [9.0, 10.0, 11.0], [10.0, 11.5, 12.5]),
            [10.0, math.nan, 12.166666666666666],
        )
    ...
    def test_null_takes_precedence_over_nan(self) -> None:
        ...
        """
        assert_matches(apply_price_typical([11.0, None], [9.0, float("nan")], [10.0, 11.5]), [10.0, None])


class TestPriceTypicalCorrectness:
    """
    ...
    def test_golden_master(self) -> None:
        ...
        assert_matches(
            ...
            [10.0, 11.166666666666666, 12.166666666666666, 11.666666666666666, 13.166666666666666],
        )


class TestPriceTypicalProperties:
    """
    ...
    def test_scale_homogeneity(
        ...
        rows = case
        high, low, close = split_triples(rows)
        result_base = apply_price_typical(high, low, close)
