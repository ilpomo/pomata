"""
Tests for ``pomata.indicators.linear_regression`` — the endpoint of the rolling least-squares line.

``linear_regression`` is single-input, so tests use the shared ``apply_expr`` helper to materialize the factory over a
one-column ``Float64`` frame; ``assert_matches`` and the naive ``linear_regression_reference`` oracle are shared across
the suite. The endpoint is homogeneous of degree 1 in the input, so it carries scale-homogeneity and large-magnitude
properties.

The ladder is the canonical one: contract, edge (window floor / warm-up / null / NaN), correctness (vs the closed-form
reference, a frozen golden master, and an exact-on-linear-input check that pins the convention — the endpoint of a
linear series is its current value), and properties. Categories are split into classes; cross-cutting categories use
...
class TestLinearRegressionContract:
    """
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(linear_regression(pl.col(COLUMN_X), 14), pl.Expr)
    ...
    def test_preserves_length_and_dtype(self) -> None:
        ...
        result = frame.select(linear_regression(pl.col(COLUMN_X), 3).alias("y"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [10.0, 11.0, 13.0, 12.0, 14.0])})
        expr = linear_regression(pl.col(COLUMN_X), 3).alias("y")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        expr = linear_regression(pl.col(COLUMN_X), 3).over(GROUP_KEY)
        grouped = frame.select(expr.alias("y"))["y"].to_list()
        group_a = apply_expr([10.0, 11.0, 13.0, 12.0], linear_regression(pl.col(COLUMN_X), 3))
        group_b = apply_expr([20.0, 22.0, 21.0, 24.0], linear_regression(pl.col(COLUMN_X), 3))
        assert_matches(grouped, group_a + group_b)


class TestLinearRegressionEdge:
    """
    ...
    def test_window_below_two_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="window must be >= 2"):
            linear_regression(pl.col(COLUMN_X), 1)

    def test_warmup_null_count(self) -> None:
        """
        Verifies the warm-up is ``window - 1`` rows.
        """
        result = apply_expr([10.0, 11.0, 13.0, 12.0, 14.0], linear_regression(pl.col(COLUMN_X), 3))
    ...
    def test_null_propagates(self) -> None:
        ...
        assert_matches(
            apply_expr(values, linear_regression(pl.col(COLUMN_X), 3)),
            linear_regression_reference(values, 3),
        )
    ...
    def test_window_exceeds_length(self) -> None:
        ...
        """
        assert_matches(apply_expr([1.0, 2.0, 3.0], linear_regression(pl.col(COLUMN_X), 5)), [None, None, None])

    def test_single_row(self) -> None:
        """
        Verifies that a one-element series is all warm-up: a window of more than one observation yields null.
        """
        assert_matches(apply_expr([42.0], linear_regression(pl.col(COLUMN_X), 2)), [None])

    def test_empty(self) -> None:
        """
        Verifies that an empty series yields an empty result.
        """
        assert_matches(apply_expr([], linear_regression(pl.col(COLUMN_X), 3)), [])

    def test_all_null(self) -> None:
        """
        Verifies that an all-null series stays null (no window ever holds the required non-null values).
        """
        assert_matches(apply_expr([None, None, None], linear_regression(pl.col(COLUMN_X), 2)), [None, None, None])
...
class TestLinearRegressionCorrectness:
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: endpoint(window=3) over the sample series.
        """
        result = apply_expr([10.0, 11.0, 13.0, 12.0, 14.0, 13.0, 15.0], linear_regression(pl.col(COLUMN_X), 3).round(4))
        assert_matches(result, [None, None, 12.8333, 12.5, 13.5, 13.5, 14.5])

    def test_endpoint_equals_value_on_linear_input(self) -> None:
        """
        Verifies that for a perfectly linear input the fitted endpoint reproduces the current value exactly.
        """
        values = [1.0, 3.0, 5.0, 7.0, 9.0, 11.0]
        result = apply_expr(values, linear_regression(pl.col(COLUMN_X), 3))
        for index in range(2, len(values)):
            value = result[index]
            assert value is not None
            assert math.isclose(
                value, values[index], rel_tol=RELATIVE_TOLERANCE_REFERENCE, abs_tol=ABSOLUTE_TOLERANCE_REFERENCE
            )


class TestLinearRegressionProperties:
    """
    ...
    def test_scale_homogeneity(
        ...
        """
        Verifies that, for positive ``k``, the endpoint is homogeneous of degree 1: ``f(k * x) == k * f(x)``. ``k`` is a
        power of two so the rescaling is lossless and cannot introduce a sub-ULP drift into the fitted endpoint.
        """
        k = 2.0**exponent
        values, window = case
        scaled_values = [value * k for value in values]
        base = apply_expr(values, linear_regression(pl.col(COLUMN_X), window))
        scaled = apply_expr(scaled_values, linear_regression(pl.col(COLUMN_X), window))
    ...
    def test_matches_reference_at_large_magnitude(
        ...
        assert_matches(
            apply_expr(scaled_values, linear_regression(pl.col(COLUMN_X), window)),
