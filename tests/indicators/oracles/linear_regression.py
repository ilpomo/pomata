"""
Naive reference oracle for ``pomata.indicators.linear_regression``.
"""
...
def reference_linear_regression(
    ...
    Returns:
        A list the same length as ``expr``: the fitted endpoint, ``None`` through the ``window - 1`` warm-up rows.
