"""
Tests for ``pomata.indicators.stochastic_slow`` — the Slow Stochastic Oscillator (%K and %D).

``stochastic_slow`` is multi-input (high, low, close) and multi-output: it returns a single struct ``pl.Expr`` with the
fields ``k`` / ``d``. The local ``apply_stochastic_slow`` helper materializes each field over a three-column ``Float64``
frame into a dict of lists, so the shared ``assert_matches`` and the naive ``stochastic_slow_reference`` oracle (which
...
from tests.indicators.oracles import stochastic_slow_reference
...
# other indicator's.
#   1. warmup  W(window_k, window_slowing, window_d) = window_k + window_slowing + window_d - 3   (the slow %K warms up
#              over ``window_k + window_slowing - 2`` rows and %D over a further ``window_d - 1`` of the sma of slow %K)
...
def _cases[T](draw: st.DrawFn, bars: st.SearchStrategy[T]) -> tuple[list[T], int, int, int]:
    """
    A (series, window_k, window_slowing, window_d) tuple sized from the facts above: each window over its regimes,
    length = warm-up + a window of defined bars, so every example has output to check (never an all-warm-up series, the
    waste a window decoupled from the length would cause).
    """
    window_k = draw(st.integers(min_value=1, max_value=WINDOW_MAX))
    window_slowing = draw(st.integers(min_value=1, max_value=WINDOW_MAX))
    window_d = draw(st.integers(min_value=1, max_value=WINDOW_MAX))
    warmup = window_k + window_slowing + window_d - 3
    defined = draw(st.integers(min_value=1, max_value=WINDOW_MAX))
    length = warmup + defined
    return draw(st.lists(bars, min_size=length, max_size=length)), window_k, window_slowing, window_d


def apply_stochastic_slow(
    ...
    """
    Materialize each line of ``stochastic_slow`` over a three-column ``Float64`` frame, as a dict mirroring the oracle.
    """
...
class TestStochasticSlowContract:
    """
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(
            stochastic_slow(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window_k=14, window_slowing=3, window_d=3),
            pl.Expr,
        )
    ...
    def test_output_is_struct_with_named_fields(self) -> None:
        ...
        dtype = frame.select(
            stochastic_slow(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window_k=2, window_slowing=2, window_d=2).alias(
                "s"
            )
        ).schema["s"]
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        group_a = apply_stochastic_slow(
            [10.0, 11.0, 12.0, 11.5, 13.0], [9.0, 10.0, 11.0, 10.5, 12.0], [9.5, 10.5, 11.5, 11.0, 12.5], 2, 2, 2
        )
        group_b = apply_stochastic_slow(
            [20.0, 21.0, 22.0, 21.5, 23.0], [19.0, 20.0, 21.0, 20.5, 22.0], [19.5, 20.5, 21.5, 21.0, 22.5], 2, 2, 2
        )
        for field in FIELDS:
            assert_matches(grouped[field], group_a[field] + group_b[field])


class TestStochasticSlowEdge:
    """
    ...
    def test_window_k_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="window_k must be >= 1"):
            stochastic_slow(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window_k=0, window_slowing=3, window_d=3)

    def test_window_slowing_below_one_raises(self) -> None:
        """
        Verifies that ``window_slowing < 1`` raises ``ValueError``.
        """
        with pytest.raises(ValueError, match="window_slowing must be >= 1"):
            stochastic_slow(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window_k=3, window_slowing=0, window_d=3)
    ...
    def test_window_d_below_one_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="window_d must be >= 1"):
            stochastic_slow(pl.col(HIGH), pl.col(LOW), pl.col(CLOSE), window_k=3, window_slowing=3, window_d=0)

    def test_empty(self) -> None:
        """
        Verifies that an empty input yields empty output on both lines.
        """
        result = apply_stochastic_slow([], [], [], window_k=3, window_slowing=2, window_d=2)
    ...
    def test_all_null(self) -> None:
        ...
        """
        result = apply_stochastic_slow(
            [None, None, None], [None, None, None], [None, None, None], window_k=2, window_slowing=2, window_d=2
        )
    ...
    def test_single_row(self) -> None:
        ...
        """
        result = apply_stochastic_slow([11.0], [9.0], [10.0], window_k=3, window_slowing=2, window_d=2)
    ...
    def test_window_exceeds_length(self) -> None:
        ...
        """
        result = apply_stochastic_slow(
            [11.0, 12.0, 13.0], [9.0, 10.0, 11.0], [10.0, 11.0, 12.0], window_k=5, window_slowing=2, window_d=2
        )
        assert_matches(result["k"], [None, None, None])
        assert_matches(result["d"], [None, None, None])
    ...
    def test_warmup_null_count(self) -> None:
        """
        Verifies that ``k`` warms up over ``window_k + window_slowing - 2`` rows and ``d`` over a further
        ``window_d - 1``.
        """
        high = [10.0, 11.0, 12.0, 11.5, 13.0, 12.5]
        low = [9.0, 10.0, 11.0, 10.5, 12.0, 11.5]
        close = [9.5, 10.5, 11.5, 11.0, 12.5, 12.0]
        result = apply_stochastic_slow(high, low, close, window_k=3, window_slowing=2, window_d=2)
        assert result["k"][:3] == [None, None, None]
        assert result["k"][3] is not None
    ...
    def test_flat_window_is_nan(self) -> None:
        ...
        """
        result = apply_stochastic_slow(
            [10.0, 10.0, 10.0], [10.0, 10.0, 10.0], [10.0, 10.0, 10.0], window_k=2, window_slowing=1, window_d=1
        )
    ...
    def test_null_propagates(self) -> None:
        ...
        """
        high = [10.0, 11.0, 12.0, None, 13.0, 12.5, 14.0, 13.5, 15.0]
        low = [9.0, 10.0, 11.0, 10.5, 12.0, 11.5, 13.0, 12.5, 14.0]
        close = [9.5, 10.5, 11.5, 11.0, 12.5, 12.0, 13.5, 13.0, 14.5]
        applied = apply_stochastic_slow(high, low, close, window_k=2, window_slowing=2, window_d=2)
        reference = stochastic_slow_reference(high, low, close, 2, 2, 2)
        for field in FIELDS:
...
class TestStochasticSlowCorrectness:
    """
    ...
    def test_matches_reference(self) -> None:
        ...
        for window_k, window_slowing, window_d in ((1, 1, 1), (3, 2, 2), (5, 3, 3), (4, 1, 2)):
            applied = apply_stochastic_slow(
                high, low, close, window_k=window_k, window_slowing=window_slowing, window_d=window_d
            )
    ...
    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: stochastic_slow(window_k=5, window_slowing=3, window_d=3) over the sample series.
        """
        high = [10.0, 11.0, 12.0, 11.5, 13.0, 12.5, 14.0, 13.5, 15.0, 14.5]
        low = [9.0, 10.0, 11.0, 10.5, 12.0, 11.5, 13.0, 12.5, 14.0, 13.5]
        close = [9.5, 10.5, 11.5, 11.0, 12.5, 12.0, 13.5, 13.0, 14.5, 14.0]
        result = apply_stochastic_slow(high, low, close, window_k=5, window_slowing=3, window_d=3)
        assert_matches(
        ...
        assert_matches(
            ...
            [None, None, None, None, None, None, None, None, 78.5053, 77.2487],
        )


class TestStochasticSlowProperties:
    """
    ...
    def test_scale_invariance(
        ...
        rows, window_k, window_slowing, window_d = case
        high, low, close = split_triples(rows)
        base = apply_stochastic_slow(
            high, low, close, window_k=window_k, window_slowing=window_slowing, window_d=window_d
        )
        scaled = apply_stochastic_slow(
