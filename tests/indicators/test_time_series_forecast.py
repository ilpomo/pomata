"""
Tests for ``pomata.indicators.time_series_forecast`` — the rolling least-squares line projected one bar ahead.

``time_series_forecast`` is single-input, so tests use the shared ``apply_expr`` helper to materialize the factory over
a one-column ``Float64`` frame; ``assert_matches`` and the naive ``time_series_forecast_reference`` oracle are shared
across the suite. The forecast is homogeneous of degree 1 in the input, so it carries scale-homogeneity and
...
reference, a frozen golden master, and an exact-on-linear-input check — the one-step forecast of a linear series is its
next value), and properties. Categories are split into classes; cross-cutting categories use markers (see
``tests/README.md``).
"""
...
class TestTimeSeriesForecastContract:
    """
    ...
    def test_returns_expr(self) -> None:
        ...
        """
        assert isinstance(time_series_forecast(pl.col(COLUMN_X), 14), pl.Expr)

    def test_preserves_length_and_dtype(self) -> None:
        """
        Verifies that the output has one value per input row and is ``Float64``.
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [10.0, 11.0, 13.0, 12.0, 14.0])})
        result = frame.select(time_series_forecast(pl.col(COLUMN_X), 3).alias("y"))
    ...
    def test_lazy_eager_parity(self) -> None:
        ...
        """
        frame = pl.DataFrame({COLUMN_X: pl.Series(COLUMN_X, [10.0, 11.0, 13.0, 12.0, 14.0])})
        expr = time_series_forecast(pl.col(COLUMN_X), 3).alias("y")
    ...
    def test_over_partitions_independently(self) -> None:
        ...
        expr = time_series_forecast(pl.col(COLUMN_X), 3).over(GROUP_KEY)
        grouped = frame.select(expr.alias("y"))["y"].to_list()
        group_a = apply_expr([10.0, 11.0, 13.0, 12.0], time_series_forecast(pl.col(COLUMN_X), 3))
        group_b = apply_expr([20.0, 22.0, 21.0, 24.0], time_series_forecast(pl.col(COLUMN_X), 3))
        assert_matches(grouped, group_a + group_b)


class TestTimeSeriesForecastEdge:
    """
    ...
    def test_window_below_two_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="window must be >= 2"):
            time_series_forecast(pl.col(COLUMN_X), 1)

    def test_warmup_null_count(self) -> None:
        """
        Verifies the warm-up is ``window - 1`` rows.
        """
        result = apply_expr([10.0, 11.0, 13.0, 12.0, 14.0], time_series_forecast(pl.col(COLUMN_X), 3))
    ...
    def test_null_propagates(self) -> None:
        ...
        """
        values = [10.0, 11.0, 13.0, None, 14.0, 14.0, 16.0]
        assert_matches(
            apply_expr(values, time_series_forecast(pl.col(COLUMN_X), 3)),
            time_series_forecast_reference(values, 3),
        )
    ...
    def test_window_exceeds_length(self) -> None:
        ...
        """
        assert_matches(apply_expr([1.0, 2.0, 3.0], time_series_forecast(pl.col(COLUMN_X), 5)), [None, None, None])

    def test_single_row(self) -> None:
        """
        Verifies that a one-element series is all warm-up: a window of more than one observation yields null.
        """
        assert_matches(apply_expr([42.0], time_series_forecast(pl.col(COLUMN_X), 2)), [None])

    def test_empty(self) -> None:
        """
        Verifies that an empty series yields an empty result.
        """
        assert_matches(apply_expr([], time_series_forecast(pl.col(COLUMN_X), 3)), [])

    def test_all_null(self) -> None:
        """
        Verifies that an all-null series stays null (no window ever holds the required non-null values).
        """
        assert_matches(apply_expr([None, None, None], time_series_forecast(pl.col(COLUMN_X), 2)), [None, None, None])


class TestTimeSeriesForecastCorrectness:
    """
    ...
    def test_matches_reference(self) -> None:
        ...
        for window in (2, 3, 4, 5):
            assert_matches(
                apply_expr(values, time_series_forecast(pl.col(COLUMN_X), window)),
                time_series_forecast_reference(values, window),
            )

    def test_golden_master(self) -> None:
        """
        Verifies the frozen reference: forecast(window=3) over the sample series.
        """
        result = apply_expr(
            [10.0, 11.0, 13.0, 12.0, 14.0, 13.0, 15.0], time_series_forecast(pl.col(COLUMN_X), 3).round(4)
        )
        assert_matches(result, [None, None, 14.3333, 13.0, 14.0, 14.0, 15.0])

    def test_forecast_equals_next_value_on_linear_input(self) -> None:
        """
        Verifies that for a perfectly linear input the one-step forecast equals the next actual value exactly.
        """
        values = [1.0, 3.0, 5.0, 7.0, 9.0, 11.0]
        result = apply_expr(values, time_series_forecast(pl.col(COLUMN_X), 3))
        for index in range(2, len(values) - 1):
            value = result[index]
            assert value is not None
            assert math.isclose(
                value, values[index + 1], rel_tol=RELATIVE_TOLERANCE_REFERENCE, abs_tol=ABSOLUTE_TOLERANCE_REFERENCE
            )


class TestTimeSeriesForecastProperties:
    """
    ...
    def test_scale_homogeneity(
        ...
        base = apply_expr(values, time_series_forecast(pl.col(COLUMN_X), window))
        scaled = apply_expr(scaled_values, time_series_forecast(pl.col(COLUMN_X), window))
    ...
    def test_matches_reference_at_large_magnitude(
        ...
        assert_matches(
            apply_expr(scaled_values, time_series_forecast(pl.col(COLUMN_X), window)),
            time_series_forecast_reference(scaled_values, window),
            rel_tol=RELATIVE_TOLERANCE_SCALE,
