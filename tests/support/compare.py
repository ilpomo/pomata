...
import them from ``tests.support.compare``; :func:`first_mismatch` is the non-raising probe the rungs use to build a
...
def first_mismatch(
    ...
    *,
    ...
    :mod:`tests.support.messages` can point at the exact row before raising. A length mismatch reports the first
...
def assert_matches(
    ...
    *,
    ...
    pass looser values explicitly (see :mod:`tests.support.tolerances`).
