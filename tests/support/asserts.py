...
suite imports them as ``from tests.support import assert_matches`` / ``assert_scale_homogeneous``.
"""
...
def assert_matches(
    ...
    *,
    ...
    floating-point error pass looser values explicitly (see :mod:`tests.support.tolerances`).

    Args:
        result_actual: The materialized output under test (e.g. from :func:`tests.support.frames.apply_expr`).
