...
``pnl``), resolves the family's :class:`~tests_new.pnl.enums.Warmup` enum to the leading-null ``int`` the generic
:class:`~tests_new.support.declaration.Declaration` and its rungs speak, assembles that declaration, registers it as a
...
from tests_new.pnl.enums import BehaviorNan, BehaviorNull, ConventionSign, NonFinite, SpaceCost, Warmup
