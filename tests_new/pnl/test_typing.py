...
so the type checkers (mypy ``--strict``, pyright, ty) confirm the inferred return type is ``pl.Expr``. The
``assert_type`` calls must be literal (they pin what the checker *infers*), so every function is spelled out here.
"""
