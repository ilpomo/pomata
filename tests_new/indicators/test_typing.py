...
type checkers (mypy ``--strict``, pyright, ty) confirm the optional-keyword overloads still resolve to ``pl.Expr``. The
``assert_type`` calls must be literal — they pin what the checker *infers* — so this family is spelled out, not
parametrized.
"""
