...
:class:`~tests_new.indicators.enums.Warmup` enum to the leading-null ``int`` / mapping / ``None`` the generic
:class:`~tests_new.support.declaration.Declaration` and its rungs speak, assembles that declaration, registers it as
...
Two family facts are stored but not yet read by any rung — :class:`~tests_new.indicators.enums.Seeding` (Phase-B
docstring metadata) — while :class:`~tests_new.indicators.enums.RelationTalib` and its reason drive the differential
tier's partition of the public surface. The escape hatches (``flow_deviation`` for the directional-movement pair,
...
from tests_new.support.registry import register
...
def suite_indicators(  # noqa: PLR0913
    *,
    ...
    Args:
        ...
        seeding: The recursion's seeding convention — Phase-B metadata, not read by any rung.
