...
canonical declaration parameters, so they cannot be derived from the registry without changing the values.

The no-twin / deliberate-divergence partition is no longer a separate registry module: it is derived from each
declaration's :class:`~tests_new.indicators.enums.RelationTalib` axis (``MATCHES`` is compared here, the other two are
accounted for but not compared), so the shared pure data lives on the declarations and cannot drift from the suite.
"""
...
from tests_new.indicators.enums import RelationTalib
from tests_new.support.registry import registry_indicators
from tests_new.support.tolerances import TOLERANCE_ABSOLUTE_REFERENCE, TOLERANCE_RELATIVE_REFERENCE

# ``all_declarations`` is imported for its registration side effects, so ``registry_indicators`` is populated before the
# partition below reads it; nothing is referenced from it directly.
del _registered

talib = pytest.importorskip("talib")

# The TA-Lib partition, derived from the declarations instead of a separate registry module: the matching twins compared
# here, the documented divergences and no-equivalents accounted for (each carrying its reason on the declaration).
DOCUMENTED_DIVERGENCES: dict[str, str] = {
    name: decl.talib_reason
    for name, decl in registry_indicators.items()
    if decl.talib is RelationTalib.DOCUMENTED_DIVERGENCE
}
NO_TALIB_EQUIVALENT: dict[str, str] = {
    name: decl.talib_reason for name, decl in registry_indicators.items() if decl.talib is RelationTalib.NO_EQUIVALENT
}
TALIB_MATCHES: set[str] = {name for name, decl in registry_indicators.items() if decl.talib is RelationTalib.MATCHES}
...
class Market:
    """A market fixture: the OHLCV series as numpy arrays (for TA-Lib) and the same data as a Polars frame (ours)."""
...
def _market(seed: int) -> Market:
    ...
    return Market(open=open_, high=high, low=low, close=close, volume=volume, frame=frame)
...
# Each spec maps an indicator name to its (label, pomata-output, TA-Lib-output) triples on a given market. The TA-Lib
# output is an untyped C-library array; multi-output indicators (e.g. macd) yield one triple per field.
Compare = Callable[[Market], list[tuple[str, list[float | None], npt.NDArray[np.float64]]]]

SPECS: dict[str, Compare] = {
    "accumulation_distribution": lambda m: [
        (
            "accumulation_distribution",
            _ours(m.frame, accumulation_distribution(pl.col("high"), pl.col("low"), pl.col("close"), pl.col("volume"))),
    ...
    "macd": lambda m: [
        (
            "macd",
            _ours(m.frame, macd(pl.col("close"), window_fast=12, window_slow=26, window_signal=9).struct.field("macd")),
    ...
    "midpoint": lambda m: [("midpoint", _ours(m.frame, midpoint(pl.col("close"), 14)), talib.MIDPOINT(m.close, 14))],
    "midprice": lambda m: [
        (
            "midprice",
    ...
    "rsi_stochastic": lambda m: [
        (
            "k",
            _ours(m.frame, rsi_stochastic(pl.col("close"), window_rsi=14, window_k=14, window_d=3).struct.field("k")),
            talib.STOCHRSI(m.close, 14, 14, 3)[0],
        ),
        (
            "d",
            _ours(m.frame, rsi_stochastic(pl.col("close"), window_rsi=14, window_k=14, window_d=3).struct.field("d")),
...
# The no-twin / deliberate-divergence partition is derived from the declarations' RelationTalib axis above, so it stays
# in exact correspondence with the suite and no separate registry module can drift out of sync.
...
def test_every_indicator_is_accounted_for() -> None:
    """
    Verifies every public indicator is either compared against TA-Lib or documented as not comparable, and that the
    declarations' RelationTalib partition agrees with the bespoke comparison table (the guard that catches a matching
    twin mislabeled no-equivalent, or vice versa).
    """
    ...
    # The RelationTalib.MATCHES declarations are in exact bijection with the bespoke comparison table: a MATCHES
    # function without a live TA-Lib comparison here (or a NO_EQUIVALENT / DOCUMENTED_DIVERGENCE that secretly has one)
    # breaks this equality.
    assert set(SPECS) == TALIB_MATCHES
    # The accounted-for partition is disjoint from the compared table and every entry carries a reason.
    assert not (set(NO_TALIB_EQUIVALENT) | set(DOCUMENTED_DIVERGENCES)) & set(SPECS)
    assert all(NO_TALIB_EQUIVALENT.values())
    assert all(DOCUMENTED_DIVERGENCES.values())
