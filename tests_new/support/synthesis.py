...
:data:`tests_new.support.columns.COLUMN_X`, ``int`` -> a small window, ``float`` -> a small factor), so a test
...
import polars as pl
from tests_new.support.columns import COLUMN_X
...
def sample_argument(annotation: object) -> object:
    ...
    ``pl.Expr`` -> a reference to :data:`tests_new.support.columns.COLUMN_X`; ``int`` -> a small window; ``float`` -> a
