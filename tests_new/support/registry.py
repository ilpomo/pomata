...
A per-family suite function registers each :class:`Declaration` as a side effect of importing its module, so the
explicit aggregator import the old suite maintained by hand disappears; the completeness guarantee stays fail-closed
(a public function without a declaration is red), but it is no longer the contributor's chore. The registry has one
