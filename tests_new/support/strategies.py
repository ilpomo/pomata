...
canonical, one-home input domains for the fuzz engine (:func:`tests_new.support.synthesis.fuzz_frames`), so the same
...
def subnormal_safe_floats(bound: float = 1e3) -> st.SearchStrategy[float]:
    ...
    (:func:`tests_new.support.tolerances.streaming_abs_tol`), which would otherwise collapse toward zero on a
