...
def validate_finite(
    ...
    Shared by the factories that take a finite scalar knob (the metrics' ``threshold``, ``risk_free_rate``, and
    ``benchmark_sharpe``, the indicators' ``volume_factor``), so a ``NaN`` or ``inf`` is rejected at the call site
    rather than silently poisoning the result.
